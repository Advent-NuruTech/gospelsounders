...
export default function Sidebar({ isOpen, onClose, publicNavItems }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-[85%] max-w-[320px] bg-[#3B2414] dark:bg-[#F6F1EA] shadow-2xl z-50 transform transition-transform duration-300 ease-out lg:hidden ${
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-300 transition-colors z-10"
            ...
            {publicNavItems.map((item) => (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 group ${
                  isActive(item.href)
                    ? "bg-[#D8C9B4] text-[#3B2414] dark:bg-[#6B4A2E] dark:text-[#F6F1EA] shadow-sm"
